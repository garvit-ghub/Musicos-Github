  ...
  return (
    <div style={containerStyle}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <h2 style={{ color: '#10b981', margin: 0 }}>User Directory</h2>
        <button onClick={fetchUsers} className="btn" style={{ background: '#3f3f46' }}>Refresh</button>
      </div>
          ...
          <tbody>
            {users.map(u => (
              <tr key={u._id} style={rowStyle}>
                ...
                <td style={tdStyle}>
                  <span style={{ background: u.role === 'admin' ? 'rgba(249,115,22,0.2)' : 'rgba(16,185,129,0.2)', color: u.role === 'admin' ? '#f97316' : '#10b981', padding: '4px 8px', borderRadius: '4px', fontSize: '0.85rem', fontWeight: 'bold' }}>
                    {u.role.toUpperCase()}
                  </span>
                </td>
                <td style={tdStyle}>{new Date(parseInt(u._id.substring(0, 8), 16) * 1000).toLocaleDateString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
